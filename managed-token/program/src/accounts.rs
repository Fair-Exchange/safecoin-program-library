...
impl<'a, 'info> InitializeMint<'a, 'info> {
    pub fn load(accounts: &'a [AccountInfo<'info>]) -> Result<Self, ProgramError> {
        ...
        assert_with_msg(
            ctx.token_program.key == &safe_token::id(),
...
impl<'a, 'info> InitializeAccount<'a, 'info> {
    pub fn load(accounts: &'a [AccountInfo<'info>]) -> Result<Self, ProgramError> {
        ...
        assert_with_msg(
            ctx.associated_token_program.key == &safe_associated_token_account::id(),
            ProgramError::InvalidInstructionData,
...
impl<'a, 'info> Transfer<'a, 'info> {
    pub fn load(accounts: &'a [AccountInfo<'info>]) -> Result<Self, ProgramError> {
        ...
        assert_with_msg(
            ctx.mint.owner == &safe_token::id(),
            ProgramError::IllegalOwner,
            "Mint account must be owned by the Token Program",
        )?;
        assert_with_msg(
            ctx.src_account.owner == &safe_token::id(),
            ProgramError::IllegalOwner,
            "Source token account must be owned by the Token Program",
        )?;
        assert_with_msg(
            ctx.dst_account.owner == &safe_token::id(),
            ProgramError::IllegalOwner,
