...
use {
    ...
    safecoin_sdk::{program_pack::Pack, pubkey::Pubkey},
...
pub fn init(client: &FarmClient, config: &Config, dao_program: &Pubkey, mint_ui_amount: f64) {
    ...
    let mint_address = Pubkey::create_with_seed(&wallet, DAO_MINT_NAME, &safe_token::id()).unwrap();
    let mint_size = safe_token::state::Mint::get_packed_len();
    ...
    if client.rpc_client.get_account_data(&mint_address).is_err() {
        ...
        let token = Token {
            ...
            description: str_to_as64("Safecoin Farms Governance Token").unwrap(),
    ...
    if !client.has_active_token_account(&custody_authority, "SAFE") {
        let wsol_token = client.get_token("SAFE").unwrap();
        inst.push(create_associated_token_account(
            &wallet,
            &custody_authority,
            &wsol_token.mint,
