...
pub fn load(client: &FarmClient, config: &Config, data: &str, remove_mode: bool) {
    ...
    if parsed.get("name").is_some() {
        if parsed["name"] == "Safecoin Token List"
            || parsed["name"] == "Saber Tokens"
            || parsed["name"] == "Raydium Mainnet Token List"
        {
            load_safecoin_tokens(client, config, remove_mode, &parsed, last_index);
...
fn load_safecoin_tokens(
    ...
    for val in tokens {
        ...
        let token_type = if git_token.symbol.to_uppercase() == "SAFE" {
            TokenType::WrappedSafe
        } else {
...
fn get_token_type_from_tags(tags: &[String]) -> TokenType {
    if tags.contains(&String::from("Safecoin tokenized")) {
        TokenType::WrappedSafe
    } else if tags.contains(&String::from("wrapped-sollet")) {
        TokenType::WrappedSafelet
...
fn get_oracle_price_account(config: &Config, symbol: &str) -> (OracleType, Pubkey) {
    let acc = if config.farm_client_url.contains("devnet") {
        match symbol {
            "SAFE" => "J83w4HKfqxwcq3BEMMkPFSppX3gqekLyLJBexebFVkix",
            "WSAFE" => "J83w4HKfqxwcq3BEMMkPFSppX3gqekLyLJBexebFVkix",
            "MSAFE" => "9a6RNx3tCu1TSs6TBSfV2XRXEPEZXQ6WB7jRojZRvyeZ",
        ...
        match symbol {
            ...
            "SAFE" => "H6ARHf6YXhGYeQfUzQNGk6rDNnLBQKrenN712K4AQJEG",
            "WSAFE" => "H6ARHf6YXhGYeQfUzQNGk6rDNnLBQKrenN712K4AQJEG",
            ...
            "MSAFE" => "E4v1BBgoso9s64TQvmyownAVJbhbEPGyzA3qn4n46qj9",
            ...
            "STSAFE" => "Bt1hEbY62aMriY1SyQqbeZbm8VmSbQVGBFzSzMuVNWzN",
