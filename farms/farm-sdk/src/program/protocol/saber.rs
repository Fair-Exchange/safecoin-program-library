...
pub fn add_liquidity(
    ...
    if let [user_account, user_token_a_account, user_token_b_account, user_lp_token_account, pool_program_id, pool_token_a_account, pool_token_b_account, lp_token_mint, _safe_token_id, _clock_id, swap_account, swap_authority] =
        accounts
    {
...
pub fn add_liquidity_with_seeds(
    ...
    if let [authority_account, token_a_custody_account, token_b_custody_account, lp_token_custody_account, pool_program_id, pool_token_a_account, pool_token_b_account, lp_token_mint, _safe_token_id, _clock_id, swap_account, swap_authority] =
        accounts
    {
        if &stable_swap_client::id() != pool_program_id.key {
            return Err(ProgramError::IncorrectProgramId);
...
pub fn remove_liquidity_with_seeds(
    ...
    if let [authority_account, token_a_custody_account, token_b_custody_account, lp_token_custody_account, pool_program_id, pool_token_a_account, pool_token_b_account, lp_token_mint, _safe_token_id, swap_account, swap_authority, fees_account_a, fees_account_b] =
        accounts
    {
        if &stable_swap_client::id() != pool_program_id.key {
            return Err(ProgramError::IncorrectProgramId);
...
pub fn stake_with_seeds(
    ...
    if let [authority_account, lp_token_custody_account, farm_program_id, _safe_token_id, miner, miner_vault, quarry, rewarder] =
        accounts
    {
        if &quarry_mine::id() != farm_program_id.key {
            return Err(ProgramError::IncorrectProgramId);
...
pub fn claim_rewards_with_seeds(accounts: &[AccountInfo], seeds: &[&[&[u8]]]) -> ProgramResult {
    if let [authority_account, iou_token_custody_account, farm_program_id, _safe_token_id, _zero_id, miner, rewarder, minter, mint_wrapper, mint_wrapper_program, iou_token_mint, iou_fees_account, quarry] =
        accounts
    {
        if &quarry_mine::id() != farm_program_id.key
...
pub fn redeem_rewards_with_seeds(accounts: &[AccountInfo], seeds: &[&[&[u8]]]) -> ProgramResult {
    if let [authority_account, iou_token_custody_account, sbr_token_custody_account, _safe_token_id, redeemer, redeemer_program, sbr_token_mint, iou_token_mint, saber_vault, saber_mint_proxy_program, mint_proxy_authority, mint_proxy_state, minter_info] =
        accounts
    {
...
pub fn swap_with_seeds(
    ...
    if let [authority_account, token_a_custody_account, token_b_custody_account, pool_program_id, pool_token_a_account, pool_token_b_account, _safe_token_id, _clock_id, swap_account, swap_authority, fees_account] =
        accounts
    {
        if &stable_swap_client::id() != pool_program_id.key {
            return Err(ProgramError::IncorrectProgramId);
