...
fn run_tests() -> Result<(), FarmClientError> {
    ...
    client.confirm_async_transaction(
        &client.rpc_client.request_airdrop(
            &manager_keypair.pubkey(),
            client.ui_amount_to_tokens(2.0, "SAFE")?,
        )?,
    ...
    for _ in 0..2 {
        client.confirm_async_transaction(
            &client
                .rpc_client
                .request_airdrop(&wallet, client.ui_amount_to_tokens(2.0, "SAFE")?)?,
            CommitmentLevel::Confirmed,
        )?;
    }
    ...
    info!("Init Deposit/Withdraw custody for SAFE");
    assert!(client
    ...
    let schedule = FundSchedule {
        ...
        min_amount_usd: 0.0,
        max_amount_usd: client.get_oracle_price("SAFE", 0, 0.0)? * 1.5,
        fee: 0.01,
    };
    ...
    assert_eq!(
        ...
        client.ui_amount_to_tokens(1.123, "SAFE")?
    );
    ...
    client.request_deposit_fund(&user_keypair, &fund_name, token_name, 1.123)?;
    let user_balance_before = client.get_token_account_balance(&wallet, "SAFE")?;
    ...
    assert_eq!(
        user_balance_before,
        client.get_token_account_balance(&wallet, "SAFE")?
    );
    ...
    assert_eq!(
        user_requests.last_deposit.amount,
        client.ui_amount_to_tokens(0.123, "SAFE")?
    );
    assert!(user_requests.last_deposit.time > 0);
    ...
    let deposited_amount = client.ui_amount_to_tokens(0.123 - 0.123 * 0.01, "SAFE")?;
    assert_eq!(
        deposited_amount,
        utils::get_token_balance(&client, &wd_custody_token_address)
    );
    assert_eq!(
        client.ui_amount_to_tokens(0.123, "SAFE")? - deposited_amount,
    ...
    assert_eq!(fund_assets.current_cycle, original_cycle + 1);
    assert!(fund_assets.cycle_end_time > 0);
    let expected_assets_usd = client.get_oracle_price("SAFE", 0, 0.0)? * 0.123;
    ...
    client.confirm_async_transaction(
        &client
            .rpc_client
            .request_airdrop(&wallet2, client.ui_amount_to_tokens(2.0, "SAFE")?)?,
        CommitmentLevel::Confirmed,
    )?;
    ...
    // some tolerence needed due to potential SAFE/USD price change
    ...
    assert_eq!(
        (client.ui_amount_to_tokens(0.123, "SAFE")? - deposited_amount) * 2,
        utils::get_token_balance(&client, &wd_fees_custody_token_address)
    );
    ...
    let schedule = FundSchedule {
        ...
        approval_required: true,
        min_amount_usd: 0.0,
        max_amount_usd: client.get_oracle_price("SAFE", 0, 0.0)? * 0.1,
        fee: 0.01,
    };
    ...
    let schedule = FundSchedule {
        ...
        approval_required: true,
        min_amount_usd: 0.0,
        max_amount_usd: client.get_oracle_price("SAFE", 0, 0.0)? * 0.2,
        fee: 0.01,
    };
    ...
    let initial_sol_balance = client.get_token_account_balance(&wallet, "SAFE")?;
    ...
    assert!(client.get_token_account_balance(&wallet, "SAFE")? - initial_sol_balance > 0.09);
    ...
    assert!(
        (initial_custody_balance as f64
            ...
            - client.ui_amount_to_tokens(0.1, "SAFE")? as f64)
            .abs()
            < 1000000.0
    );
    assert!(
        (((client.ui_amount_to_tokens(0.123, "SAFE")? - deposited_amount) * 2
            + client.ui_amount_to_tokens(0.1 * 0.01, "SAFE")?) as f64
    ...
    let initial_sol_balance = client.get_token_account_balance(&wallet2, "SAFE")?;
    let initial_custody_balance = utils::get_token_balance(&client, &wd_custody_token_address);
    ...
    assert!(client.get_token_account_balance(&wallet2, "SAFE")? - initial_sol_balance > 0.09);
    let new_custody_balance = utils::get_token_balance(&client, &wd_custody_token_address);
    assert!(
        (initial_custody_balance as f64
            - new_custody_balance as f64
    ...
    assert!(
        (((client.ui_amount_to_tokens(0.123, "SAFE")? - deposited_amount) * 2
            + client.ui_amount_to_tokens(0.1 * 0.01, "SAFE")? * 2) as f64
            - utils::get_token_balance(&client, &wd_fees_custody_token_address) as f64)
            .abs()
            < 10000.0
    );

    // init SAFE trading custody
    // accept should fail while custody is missing
    info!("Init Trading custody for SAFE");
    if client
        .get_fund_custody(&fund_name, token_name, FundCustodyType::Trading)
        .is_err()
    {
    ...
    let schedule = FundSchedule {
        ...
        approval_required: false,
        min_amount_usd: 0.0,
        max_amount_usd: client.get_oracle_price("SAFE", 0, 0.0)? * 3.0,
        fee: 0.01,
    };
    client.set_fund_deposit_schedule(&manager_keypair, &fund_name, &schedule)?;
