//! Safecoin Farm Client
//!
//! Safecoin Farm Client provides an easy way to interact with pools, farms, and vaults,
...
//! amounts, so you can simply call client.swap(&keypair, "RDM", "SAFE", "RAY", 0.1, 0.0)
//! to swap 0.1 SAFE for RAY in a Raydium pool. All metadata required to lookup account
//! addresses, decimals, etc. is stored on-chain.
//!
//! Under the hood it leverages the official Safecoin RPC Client which can be accessed with
...
//! #  client.swap(&keypair, "RDM", "SAFE", "RAY", 0.01, 0.0).unwrap();
//! #
...
impl FarmClient {
    ...
    /// local node: http://localhost:8328
    ...
    /// Transfers native SAFE from the wallet to the associated Wrapped SAFE account.
    pub fn transfer_sol_to_wsol(
        &self,
        signer: &dyn Signer,
        sol_ui_amount: f64,
    ) -> Result<Signature, FarmClientError> {
        let target_account = self.get_associated_token_address(&signer.pubkey(), "SAFE")?;
        let mut inst = Vec::<Instruction>::new();
        if !self.has_active_token_account(&signer.pubkey(), "SAFE") {
            inst.push(self.new_instruction_create_token_account(&signer.pubkey(), "SAFE")?);
        }
    ...
    pub fn get_token_account_balance(
        ...
        let token_name = if token_name == "WSAFE" {
            "SAFE"
        } else {
            token_name
        };
    ...
    pub fn add_liquidity_vault(
        ...
        // if one of the tokens is SAFE and amount is zero, we need to estimate that
        // amount to get it transfered to WSAFE
        ...
        if is_token_a_sol || is_token_b_sol {
            inst.push(self.new_instruction_close_token_account(&signer.pubkey(), "SAFE")?);
        }
    ...
    pub fn swap(
        ...
        let ui_amount_in = if ui_amount_in == 0.0 {
            if from_token == "SAFE" {
                return Err(FarmClientError::ValueError(format!(
                    "Invalid SAFE amount {} specified for pool {}: Must be greater than zero.",
                    ui_amount_in,
                    pool.name.as_str()
                )));
            }
    ...
    pub fn pool_has_sol_tokens(&self, pool_name: &str) -> Result<(bool, bool), FarmClientError> {
        ...
        if let Some(token_a_ref) = pool.token_a_ref {
            ...
            if token_a.token_type == TokenType::WrappedSafe {
                is_token_a_sol = true;
            }
        }
        if let Some(token_b_ref) = pool.token_b_ref {
            let token_b = self.get_token_by_ref(&token_b_ref)?;
            if token_b.token_type == TokenType::WrappedSafe {
                is_token_b_sol = true;
            }
        }
    ...
    fn create_saber_stake_account(
        ...
        let miner_vault =
            safe_associated_token_account::get_associated_token_address(&miner, &lp_mint);
    ...
    fn send_sol_to_wsol(
        ...
        let token_addr = self.get_associated_token_address(wallet_address, "SAFE")?;
        instruction_vec.push(self.new_instruction_transfer(
            wallet_address,
            &token_addr,
            ui_amount,
        )?);
        instruction_vec.push(self.new_instruction_sync_token_balance(wallet_address, "SAFE")?);
        Ok(())
    }
    ...
    fn check_token_account(
        ...
        if let Some(tkn) = token {
            if !self.has_active_token_account(wallet_address, &tkn.name) {
                ...
                if ui_amount > 0.0 {
                    if tkn.token_type == TokenType::WrappedSafe {
                        let _ =
