...
//!
//! Safecoin Farm Client provides an easy way to interact with pools, farms, vaults, and funds,
...
//! amounts, so you can simply call client.swap(&keypair, Protocol::Orca, "SAFE", "USDC", 0.1, 0.0)
...
//! #
//! #  // get oracle price
//! #  client.get_oracle_price("SAFE", 0, 0.0).unwrap();
//! #
//! #  // list official program IDs
//! #  client.get_program_ids().unwrap();
//! #
//! #  // swap in the Raydium pool
//! #  client.swap(&keypair, Protocol::Raydium, "SAFE", "RAY", 0.01, 0.0).unwrap();
//! #
...
impl FarmClient {
    ...
    pub fn check_ata_owner(
        ...
        if token_account.owner != safe_token::id() {
            return Ok(false);
        }
    ...
    fn check_fund_accounts(
        ...
        let ui_amount = if ui_amount == 0.0 && token_name == "SAFE" {
            ...
            let to_leave = self.tokens_to_ui_amount(min_balance + fees, "SAFE")?;
            if balance > to_leave {
                balance - to_leave
            } else {
                0.0
