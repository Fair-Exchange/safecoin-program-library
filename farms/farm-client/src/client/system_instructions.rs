//! Safecoin Farm Client System Instructions
...
impl FarmClient {
    ...
    /// Creates the native SAFE transfer instruction
    pub fn new_instruction_transfer(
        ...
        if let Ok(account) = self.rpc_client.get_account(destination_wallet) {
            if destination_wallet != &self.get_associated_token_address(wallet_address, "SAFE")?
                && (account.owner != system_program::id() || !account.data.is_empty())
            {
                return Err(FarmClientError::ValueError(
                    "Destination account is not a SAFE wallet".to_string(),
                ));
            }
        }
    ...
    pub fn new_instruction_token_transfer(
        ...
        Ok(safe_token_instruction::transfer(
            &safe_token::id(),
            &token_addr,
            &destination_address,
            wallet_address,
    ...
    pub fn new_instruction_sync_token_balance(
        ...
        let token_addr = self.get_associated_token_address(wallet_address, token_name)?;
        Ok(safe_token_instruction::sync_native(
            &safe_token::id(),
            &token_addr,
        )?)
    }
    ...
    pub fn new_instruction_close_token_account(
        ...
        let token_addr = self.get_associated_token_address(wallet_address, token_name)?;
        Ok(safe_token_instruction::close_account(
            &safe_token::id(),
            &token_addr,
            wallet_address,
            wallet_address,
    ...
    /// Creates a new complete set of instructions for SAFE wrapping
    pub fn all_instructions_wrap_sol(
        &self,
        wallet_address: &Pubkey,
        ui_amount: f64,
    ) -> Result<Vec<Instruction>, FarmClientError> {
        let target_account = self.get_associated_token_address(wallet_address, "SAFE")?;
        let mut inst = vec![];
        if !self.has_active_token_account(wallet_address, "SAFE") {
            inst.push(self.new_instruction_create_token_account(wallet_address, "SAFE")?);
        } else {
            self.check_ata_owner(wallet_address, "SAFE")?;
        }
        inst.push(self.new_instruction_transfer(wallet_address, &target_account, ui_amount)?);
        Ok(inst)
    }

    /// Creates a new complete set of instructions for SAFE unwrapping
    pub fn all_instructions_unwrap_sol(
        &self,
        wallet_address: &Pubkey,
    ) -> Result<Vec<Instruction>, FarmClientError> {
        let inst = vec![self.new_instruction_close_token_account(wallet_address, "SAFE")?];
        Ok(inst)
    }
