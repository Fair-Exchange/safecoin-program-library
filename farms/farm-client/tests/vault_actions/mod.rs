use {
    ...
    safecoin_sdk::{commitment_config::CommitmentConfig, signature::Keypair, signer::Signer},
    std::{thread, time},
};

const MAX_SAFE_BALANCE_TO_USE: f64 = 0.1;
...
pub fn run_test(vault_name: &str, swaps: Vec<Swap>, cleanup_swaps: Vec<Swap>) {
    ...
    let token_a_balance = if token_a_str == "SAFE" {
        MAX_SAFE_BALANCE_TO_USE.min(utils::get_token_or_native_balance(
            &client,
            &wallet,
            &token_a_str,
        ))
    ...
    let token_b_balance = if token_b_str == "SAFE" {
        MAX_SAFE_BALANCE_TO_USE.min(utils::get_token_or_native_balance(
            &client,
            &wallet,
            &token_b_str,
