...
lazy_static! {
    pub static ref CONFIG_FILE: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend(&[".config", "safecoin", "farm", "rpc_config.yml"]);
...
impl Default for Config {
    fn default() -> Self {
        ...
        let farm_client_url = "http://127.0.0.1:8328".to_string();
        let max_threads = 4;
...
mod test {
    ...
    fn test_default() {
        ...
        assert_eq!(config.farm_client_url, "http://127.0.0.1:8328");
    ...
    fn test_load_save() {
        let config = Config {
            ...
            farm_client_url: "http://test_farm:8328".to_string(),
        };
