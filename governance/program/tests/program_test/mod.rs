...
impl GovernanceProgramTest {
    ...
    pub async fn with_governed_program(&mut self) -> GovernedProgramCookie {
        ...
        // Load safecoin_bpf_rust_upgradeable program taken from safecoin test programs
        let path_buf = find_file("safecoin_bpf_rust_upgradeable.so").unwrap();
    ...
    pub async fn with_transfer_tokens_transaction(
        ...
        let mut instruction = safe_token::instruction::transfer(
            &safe_token::id(),
            &governed_token_cookie.address,
            &token_account_keypair.pubkey(),
            &proposal_cookie.account.governance,
    ...
    pub async fn with_upgrade_program_transaction(
        ...
        // Load safecoin_bpf_rust_upgraded program taken from safecoin test programs
        let path_buf = find_file("safecoin_bpf_rust_upgraded.so").unwrap();
        let program_data = read_file(path_buf);
    ...
    #[allow(dead_code)]
    pub async fn get_mint_account(&mut self, address: &Pubkey) -> safe_token::state::Mint {
        self.get_packed_account(address).await
    }
