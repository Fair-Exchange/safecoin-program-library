...
static VALID_TOKEN_PROGRAM_IDS: [Pubkey; 2] = [safe_token_2022::ID, safe_token::ID];
...
async fn command_set_transfer_fee(
    ...
    let token = token_client_from_config(config, &token_pubkey, Some(decimals))?;
    let maximum_fee = safe_token::ui_amount_to_amount(maximum_fee, decimals);
    let res = token
        .set_transfer_fee(
            &transfer_fee_authority,
            transfer_fee_basis_points,
...
async fn command_create_account(
    ...
    if immutable_owner {
        if config.program_id == safe_token::id() {
            return Err(format!(
...
async fn command_transfer(
    ...
    let maybe_transfer_balance =
        ui_amount.map(|ui_amount| safe_token::ui_amount_to_amount(ui_amount, mint_info.decimals));
    ...
    let transfer_balance = if !config.sign_only {
        ...
        println_display(
            ...
            format!(
                "Transfer {} tokens\n  Sender: {}\n  Recipient: {}",
                safe_token::amount_to_ui_amount(transfer_balance, mint_info.decimals),
                sender,
                recipient
            ),
        );
        ...
        if transfer_balance > sender_balance {
            return Err(format!(
                ...
                safe_token_2022::amount_to_ui_amount_string_trimmed(
                    sender_balance,
                    mint_info.decimals
                )
            )
    ...
    let maybe_fee =
        ui_fee.map(|ui_amount| safe_token::ui_amount_to_amount(ui_amount, mint_info.decimals));
...
async fn command_burn(
    ...
    let amount = safe_token::ui_amount_to_amount(ui_amount, mint_info.decimals);
    let decimals = if use_unchecked_instruction {
        None
    } else {
        Some(mint_info.decimals)
    ...
    if !config.sign_only {
        ...
        if account_data.lamports == 0 {
            if use_associated_account {
                ...
                return Err(format!("No wrapped SAFE in {}", account).into());
            }
        }

        println_display(
            config,
            format!("  Amount: {} SAFE", lamports_to_sol(account_data.lamports)),
        );
    }
...
async fn command_accounts(
    ...
    let filters = if let Some(token_pubkey) = maybe_token {
        ...
        vec![
            TokenAccountsFilter::ProgramId(safe_token::id()),
            TokenAccountsFilter::ProgramId(safe_token_2022::id()),
        ]
    };
    ...
    App::new(crate_name!())
            ...
            SubCommand::with_name(CommandName::Wrap.into())
                ...
                .arg(
                    Arg::with_name("create_aux_account")
                        ...
                        .help("Wrap SAFE in an auxiliary account instead of associated token account"),
                )
                .arg(
                    Arg::with_name("immutable")
                        .long("immutable")
        ...
        .subcommand(
            SubCommand::with_name(CommandName::AccountInfo.into())
                .about("Query details of an SPL Token account by address (DEPRECATED: use `safe-token display`)")
        ...
        .subcommand(
            SubCommand::with_name(CommandName::MultisigInfo.into())
                .about("Query details of an SPL Token multisig account by address (DEPRECATED: use `safe-token display`)")
                .setting(AppSettings::Hidden)
                .arg(
                    Arg::with_name("address")
                    .validator(is_valid_pubkey)
        ...
        .subcommand(
            SubCommand::with_name(CommandName::Gc.into())
                ...
                .arg(
                    ...
                    .takes_value(false)
                    .help("close all empty associated token accounts (to get SAFE back)")
                )
        )
        .subcommand(
...
async fn main() -> Result<(), Error> {
    let default_decimals = format!("{}", safe_token_2022::native_mint::DECIMALS);
        ...
        (CommandName::Gc, arg_matches) => {
            match config.output_format {
                OutputFormat::Json | OutputFormat::JsonCompact => {
                    eprintln!(
                        "`safe-token gc` does not support the `--ouput` parameter at this time"
                    );
                    exit(1);
                }
                _ => {}
...
mod tests {
    use {
        ...
        safe_token_2022::{extension::non_transferable::NonTransferable, state::Multisig},
        safe_token_client::client::{
    ...
    async fn new_validator_for_test() -> (TestValidator, Keypair) {
        ...
        test_validator_genesis.add_programs_with_path(&[
            ProgramInfo {
                ...
                program_path: PathBuf::from("../../target/deploy/safe_token.so"),
            },
            ProgramInfo {
                program_id: safe_associated_token_account::id(),
                loader: bpf_loader::id(),
                program_path: PathBuf::from("../../target/deploy/safe_associated_token_account.so"),
            },
            ProgramInfo {
                program_id: safe_token_2022::id(),
                loader: bpf_loader::id(),
                program_path: PathBuf::from("../../target/deploy/safe_token_2022.so"),
            },
        ]);
    ...
    async fn do_create_native_mint(config: &Config<'_>, program_id: &Pubkey, payer: &Keypair) {
        if program_id == &safe_token_2022::id() {
            let native_mint = safe_token_2022::native_mint::id();
    ...
    async fn create_token_default() {
        ...
        for program_id in VALID_TOKEN_PROGRAM_IDS.iter() {
            ...
            let result = process_test_command(
                &config,
                &payer,
                &["safe-token", CommandName::CreateToken.into()],
            )
            .await;
    ...
    async fn create_token_interest_bearing() {
        let (test_validator, payer) = new_validator_for_test().await;
        let config =
            test_config_with_default_signer(&test_validator, &payer, &safe_token_2022::id());
        ...
        let extension = mint_account
            ...
            .unwrap();
        assert_eq!(account.owner, safe_token_2022::id());
    ...
    async fn supply() {
        ...
        for program_id in VALID_TOKEN_PROGRAM_IDS.iter() {
            ...
            let result = process_test_command(
                &config,
                &payer,
                &["safe-token", CommandName::Supply.into(), &token.to_string()],
            )
            .await;
            let value: serde_json::Value = serde_json::from_str(&result.unwrap()).unwrap();
    ...
    async fn balance() {
        ...
        for program_id in VALID_TOKEN_PROGRAM_IDS.iter() {
            ...
            let result = process_test_command(
                &config,
                &payer,
                &["safe-token", CommandName::Balance.into(), &token.to_string()],
            )
            .await;
            let value: serde_json::Value = serde_json::from_str(&result.unwrap()).unwrap();
            assert_eq!(value["amount"], "0");
    ...
    async fn accounts() {
        ...
        for program_id in VALID_TOKEN_PROGRAM_IDS.iter() {
            ...
            let result = process_test_command(
                &config,
                &payer,
                &["safe-token", CommandName::Accounts.into()],
            )
            .await
            .unwrap();
    ...
    async fn wrap() {
        ...
        for program_id in VALID_TOKEN_PROGRAM_IDS.iter() {
            ...
            let _result = process_test_command(
                &config,
                &payer,
                &["safe-token", CommandName::Wrap.into(), "0.5"],
            )
            .await
            .unwrap();
            let account = get_associated_token_address_with_program_id(
    ...
    async fn allow_non_system_account_recipient() {
        let (test_validator, payer) = new_validator_for_test().await;
        let config = test_config_with_default_signer(&test_validator, &payer, &safe_token::id());
    ...
    async fn gc() {
        ...
        for program_id in VALID_TOKEN_PROGRAM_IDS.iter() {
            ...
            let _result =
                process_test_command(&config, &payer, &["safe-token", CommandName::Gc.into()])
                    .await
                    .unwrap();
    ...
    async fn required_transfer_memos() {
        let (test_validator, payer) = new_validator_for_test().await;
        let program_id = safe_token_2022::id();
        let config = test_config_with_default_signer(&test_validator, &payer, &program_id);
        let token = create_token(&config, &payer).await;
        let token_account =
            create_associated_account(&config, &payer, &token, &payer.pubkey()).await;
    ...
    async fn transfer_fee() {
        ...
        let withheld_amount =
            safe_token::amount_to_ui_amount(u64::from(extension.withheld_amount), TEST_DECIMALS);
        ...
        let account = config.rpc_client.get_account(&token_account).await.unwrap();
        let account_state = StateWithExtensionsOwned::<Account>::unpack(account.data).unwrap();
        let burn_amount = safe_token::amount_to_ui_amount(account_state.base.amount, TEST_DECIMALS);
        process_test_command(
            &config,
            &payer,
            &[
        ...
        let new_maximum_fee = safe_token::ui_amount_to_amount(new_maximum_fee, TEST_DECIMALS);
        assert_eq!(
            u64::from(extension.newer_transfer_fee.maximum_fee),
            new_maximum_fee
        );
