...
mod bench;
use bench::*;
use safe_token_2022::generic_token_account::GenericTokenAccount;
...
async fn command_transfer(
    ...
    if let Some(text) = memo {
        instructions.push(safe_memo::build_memo(
            text.as_bytes(),
            &[&config.fee_payer.pubkey()],
        ));
    }
...
fn native_mint(program_id: &Pubkey) -> Result<Pubkey, Error> {
    if program_id == &safe_token_2022::id() {
        Ok(safe_token_2022::native_mint::id())
    } else if program_id == &safe_token::id() {
        Ok(safe_token::native_mint::id())
    } else {
...
async fn command_required_transfer_memos(
    ...
    if enable_memos {
        instructions.push(
            safe_token_2022::extension::memo_transfer::instruction::enable_required_transfer_memos(
                &config.program_id,
                &token_account_address,
                &owner,
                &config.multisigner_pubkeys,
        ...
        instructions.push(
            safe_token_2022::extension::memo_transfer::instruction::disable_required_transfer_memos(
                &config.program_id,
                &token_account_address,
                &owner,
                &config.multisigner_pubkeys,
...
mod tests {
    ...
    async fn create_token_default() {
        ...
        for program_id in [safe_token::id(), safe_token_2022::id()] {
    ...
    async fn create_token_interest_bearing() {
        let (test_validator, payer) = new_validator_for_test().await;
        let config = test_config(&test_validator, &payer, &safe_token_2022::id());
        ...
        let account = config.rpc_client.get_account(&mint).await.unwrap();
        let mint_account =
            StateWithExtensionsOwned::<safe_token_2022::state::Mint>::unpack(account.data).unwrap();
    ...
    async fn failing_to_allow_non_system_account_recipient() {
        let (test_validator, payer) = new_validator_for_test().await;
        let config = test_config(&test_validator, &payer, &safe_token::id());
