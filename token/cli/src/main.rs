...
mod tests {
    use {
        super::*,
        safecoin_client::blockhash_query::Source,
        safecoin_sdk::{
            bpf_loader,
            signature::{Keypair, Signer},
        },
    ...
    fn create_token_default() {
        ...
        let result = process_test_command(&config, &payer, &["safe-token", "create-token"]);
    ...
    fn supply() {
        ...
        let result = process_test_command(
            &config,
            &payer,
            &["safe-token", "supply", &token.to_string()],
        );
        let value: serde_json::Value = serde_json::from_str(&result.unwrap()).unwrap();
    ...
    fn create_account_default() {
        ...
        let result = process_test_command(
            &config,
            &payer,
            &["safe-token", "create-account", &token.to_string()],
        );
        result.unwrap();
    }
    ...
    fn account_info() {
        ...
        let result = process_test_command(
            &config,
            &payer,
            &["safe-token", "account-info", &token.to_string()],
        );
        let value: serde_json::Value = serde_json::from_str(&result.unwrap()).unwrap();
        let account = get_associated_token_address_with_program_id(
            &payer.pubkey(),
    ...
    fn balance() {
        ...
        let result = process_test_command(
            &config,
            &payer,
            &["safe-token", "balance", &token.to_string()],
        );
        let value: serde_json::Value = serde_json::from_str(&result.unwrap()).unwrap();
        assert_eq!(value["amount"], "0");
        assert_eq!(value["uiAmountString"], "0");
    ...
    fn mint() {
        ...
        let result = process_test_command(
            &config,
            &payer,
            &["safe-token", "mint", &token.to_string(), "100"],
        );
        result.unwrap();
    ...
    fn accounts() {
        ...
        let result = process_test_command(&config, &payer, &["safe-token", "accounts"]).unwrap();
    ...
    fn wrap() {
        let (test_validator, payer) = validator_for_test();
        let config = test_config(&test_validator, &payer, &safe_token::id());
        let _result = process_test_command(&config, &payer, &["safe-token", "wrap", "0.5"]).unwrap();
        let account = get_associated_token_address_with_program_id(
            &payer.pubkey(),
            &native_mint::id(),
            &config.program_id,
    ...
    fn unwrap() {
        ...
        let result = process_test_command(
            &config,
            &payer,
            &["safe-token", "unwrap", &account.to_string()],
        );
        result.unwrap();
    ...
    fn gc() {
        let (test_validator, payer) = validator_for_test();
        let mut config = test_config(&test_validator, &payer, &safe_token::id());
        ...
        let _result = process_test_command(&config, &payer, &["safe-token", "gc"]).unwrap();
    ...
    fn set_owner() {
        ...
        let _result = process_test_command(
            &config,
            &payer,
            &["safe-token", "authorize", &aux_string, "owner", &aux_string],
        )
        .unwrap();
