...
use {
    ...
    safe_token_client::token::TokenError as TokenClientError,
    std::sync::Arc,
};
...
async fn test_memo_transfers(
    ...
    let mut memo_ix = safe_memo::build_memo(&[240, 159, 166, 150], &[]);
    for program_id in [safe_memo::id(), safe_memo::v1::id()] {
        ...
        let instructions = vec![
            ...
            safe_token_2022::instruction::transfer(
                &safe_token_2022::id(),
                &alice_account,
                &bob_account,
                &alice.pubkey(),
    ...
    let mut ctx = context.lock().await;
    memo_ix.program_id = safe_memo::v1::id();
    #[allow(deprecated)]
    let instructions = vec![
        memo_ix,
        safe_token_2022::instruction::transfer(
