...
use {
    ...
    safe_token_2022::{
        ...
        processor::Processor as SafeToken2022Processor,
    },
    safe_token_client::{
...
async fn make_context() -> TestContext {
    // TODO this may be removed when we upgrade to a safecoin version with a fixed `get_stack_height()` stub
    ...
    let mut program_test = ProgramTest::new(
        "safe_token_2022",
        safe_token_2022::id(),
        processor!(SafeToken2022Processor::process),
    );

    program_test.add_program(
        "safe_instruction_padding",
        safe_instruction_padding::id(),
        processor!(safe_instruction_padding::processor::process),
    );
...
async fn test_cpi_guard_set_authority() {
    ...
    for state in states {
        ...
        let instruction = if do_in_cpi {
            wrap_instruction(safe_instruction_padding::id(), token_instruction, vec![], 0).unwrap()
        } else {
            token_instruction
        };
