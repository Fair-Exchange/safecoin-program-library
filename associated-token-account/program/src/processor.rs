...
use crate::*;
use crate::{instruction::AssociatedTokenAccountInstruction, tools::account::create_pda_account, tools::account::get_account_len};
...
pub fn process_create_associated_token_account(
    ...
    if let Some(rent_sysvar_info) = rent_sysvar_info {
        ...
        invoke(
            &safe_token::instruction::initialize_account3(
                safe_token_program_id,
                associated_token_account_info.key,
                safe_token_mint_info.key,
                wallet_account_info.key,
