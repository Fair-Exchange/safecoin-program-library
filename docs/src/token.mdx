...
#### npm
```console
npm install @safecoin/safe-token
```
### Configuration
You can connect to different clusters using `Connection` in `@safecoin/web3.js`
```typescript
const web3 = require('@safecoin/web3.js');
const connection = new web3.Connection(web3.clusterApiUrl('devnet'), 'confirmed');
```
### Keypair
You can either get your keypair using [`Keypair`](https://solana-labs.github.io/solana-web3.js/classes/Keypair.html) from `@safecoin/web3.js`, or let the user's wallet handle the keypair and use `sendTransaction` from [`wallet-adapter`](https://github.com/solana-labs/wallet-adapter)
...
```jsx
import { createMint } from '@safecoin/safe-token';
import { clusterApiUrl, Connection, Keypair, LAMPORTS_PER_SAFE } from '@safecoin/web3.js';
...
When you want to wrap SAFE, you can send SAFE to an associated token account
on the native mint and call `syncNative`. `syncNative` updates the `amount`
field on the token account to match the amount of wrapped SAFE available.
That SAFE is only retrievable by closing the token account and choosing
...
```jsx
import {NATIVE_MINT, createAssociatedTokenAccountInstruction, getAssociatedTokenAddress, createSyncNativeInstruction, getAccount} from "@safecoin/safe-token";
import {clusterApiUrl, Connection, Keypair, LAMPORTS_PER_SAFE, SystemProgram, Transaction, sendAndConfirmTransaction} from "@safecoin/web3.js";
...
// Create token account to hold your wrapped SAFE
...
// Transfer SAFE to associated token account and use SyncNative to update wrapped SAFE balance
...
console.log(`Balance before unwrapping 1 WSAFE: ${walletBalance}`)
...
console.log(`Balance after unwrapping 1 WSAFE: ${walletBalancePostClose}`)

/*
Balance before unwrapping 1 WSAFE: 997950720
Balance after unwrapping 1 WSAFE: 1999985000
*/
```
...
```jsx
import { clusterApiUrl, Connection, Keypair, LAMPORTS_PER_SAFE } from '@safecoin/web3.js';
import { createMint, getOrCreateAssociatedTokenAccount, mintTo, transfer } from '@safecoin/safe-token';
...
```console
$ safe-token transfer AQoKYV7tYpTrFZN6P5oUufbQKAUr9mNYGe1TTJC9wajM 50 CqAxDdBRnawzx9q4PYM3wrybLHBhDZ4P6BTV13WsRJYJ
Transfer 50 tokens
  Sender: 7UX2i7SucgLMQcfZ75s3VXmZZY4YRUyJN9X1RgfMoDUi
...
```jsx
import {getAccount, createMint, createAccount, mintTo, getOrCreateAssociatedTokenAccount, transfer} from "@safecoin/safe-token";
import {clusterApiUrl, Connection, Keypair, LAMPORTS_PER_SAFE} from "@safecoin/web3.js";
...
<ul>
  <li>Mint account mint authority:
    <code>safe-token mint ...</code>,
    <code>safe-token authorize ... mint ...</code>
  </li>
  <li>Mint account freeze authority:
    <code>safe-token freeze ...</code>,
    <code>safe-token thaw ...</code>,
    <code>safe-token authorize ... freeze ...</code>
  </li>
  <li>Token account owner authority:
    <code>safe-token transfer ...</code>,
    <code>safe-token approve ...</code>,
    <code>safe-token revoke ...</code>,
    <code>safe-token burn ...</code>,
    <code>safe-token wrap ...</code>,
    <code>safe-token unwrap ...</code>,
    <code>safe-token authorize ... owner ...</code>
  </li>
  <li>Token account close authority:
    <code>safe-token close ...</code>,
    <code>safe-token authorize ... close ...</code>
  </li>
</ul>
