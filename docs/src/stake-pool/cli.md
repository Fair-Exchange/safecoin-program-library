...
If a manager would like to restrict deposits (stake and SAFE) to one key in
...
As long as the deposit authority is set, SAFE and stake deposits must be signed
...
a particular signature on stake deposits, SAFE deposits, or SAFE withdrawals. This
does not make the pool private, since all information is available on-chain, but
it restricts who can use the pool.

As an example, let's say a pool wants to restrict all SAFE withdrawals.
...
sign all SAFE withdrawals, otherwise the operation fails.

After some time, if the manager wishes to enable SAFE withdrawals, they can remove
the restriction:
...
Now, anyone can withdraw SAFE from the stake pool, provided there is enough SAFE left
in the reserve.
...
The SAFE used to add validators to the pool comes from the stake pool's reserve
account. If there is insufficient SAFE in the reserve, the command will fail.
Be sure to use the `deposit-sol` command to move some SAFE into the pool.

With 10 SAFE in the pool, let's add some random validators to the stake pool.
...
We can see the status of a stake account using the Safecoin command-line utility.

```console
$ safecoin stake-account 5AaobwjccyHnXhFCd24uiX6VqPjXE3Ry4o92fJjqqjAr
...
SAFE Deposit Fee: none
SAFE Deposit Referral Fee: none
...
actually has an additional balance of 0.002282881 SAFE, but since this is the minimum
...
```console
$ safecoin stake-account nHEEyey8KkgHuVRAUDzkH5Q4PkA4veSHuTxgG6C8L2G
...
SAFE to be distributed evenly, meaning `40` in each account. They need
...
`EhRbKi4Vhm1oUCGWHiLEMYZqDrHwEd7Jgzgi26QJKvfQ`, by a total of `60` SAFE.
...
Internally, this instruction splits and deactivates 60 SAFE from the
...
They add 30 SAFE to `J3xu64PWShcMen99kU3igxtwbke2Nwfo8pkZNRgrq66H`:
...
And they add 30 SAFE to `38DYMkwYCvsj8TC6cNaEvFHHVDYeWDp1qUgMgyjNqZXk`:
...
Stake pools accept SAFE deposits directly from a normal SAFE wallet account, and
...
```console
$ safe-token balance BoNneHKDrX9BHjjvSpPfnQyRjsnc9WFH71v8wrgCd7LB
100.00000000
```

### Withdraw SAFE

Stake pools allow SAFE withdrawals directly from the reserve and into a normal
SAFE wallet account, and in exchange burns the provided pool tokens.
...
The stake pool has burned 2 pool tokens, and in return, sent SAFE to
...
```console
$ safecoin balance 7VXPpSxneL6JLj18Naw2gkukXtjBZfbmPh18cnoUCMD8
2 SAFE
```
...
$ safecoin delegate-stake 97wBBiLVA7fUViEew8yV8R6tTdKithZDVz8LHLfF9sTJ 38DYMkwYCvsj8TC6cNaEvFHHVDYeWDp1qUgMgyjNqZXk
...
Stake pools have separate fees for stake and SAFE, so the total fee from depositing
a stake account is calculated from the rent-exempt reserve as SAFE, and the delegation
as stake.

For example, if a stake pool has a stake deposit fee of 1%, and a SAFE deposit fee
of 5%, and you deposit a stake account with 10 SAFE in stake, and .00228288 SAFE
...
Let's withdraw active staked SAFE in exchange for 5 pool tokens.
...
```console
$ safecoin stake-account 5GuAyPAt6577HoGhSVRNBv6aHohVtjQ8q7q5i3X1p4tB
...
users to reclaim their SAFE by keeping everything at the minimum amount.
...
```console
$ spl-stake-pool withdraw-stake Zg5YBPAk8RqBR9kaLLSoN5C8Uv7nErBz1WC63HTsCPR 1.00228288 SAFE
