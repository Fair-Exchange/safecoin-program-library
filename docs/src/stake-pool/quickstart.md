...
This guide requires the Safecoin CLI tool suite and Stake Pool CLI tool.

- [Install the Safecoin Tools](https://docs.solana.com/cli/install-safecoin-cli-tools)
- [Install the Stake Pool CLI](cli.md)

You must also have an account with SAFE. The guide will assume that you
are using the default keypair created at the default location using `safecoin-keygen new`.
Note that it is possible to override the default keypair with every command if
needed.

If you are running on localhost using `safecoin-test-validator`, the default keypair
will automatically start with 500,000,000 SAFE.

If you are running on devnet or testnet, you can airdrop funds using `safecoin airdrop 1`.
...
This guide uses the
[sample scripts on GitHub](https://github.com/fair-exchange/safecoin-program-library/tree/master/stake-pool/cli/scripts)
...
* `deposit.sh`: performs stake and SAFE deposits
...
Note: this will fail if another `safecoin-test-validator` is already running.
...
need to use the secret `--force` flag with `safecoin delegate-stake`, ie:
...
run the script with the amount of SAFE to deposit. We'll use 15 SAFE:
...
Depositing SAFE into stake pool
Update not required
...
this phase is ~2.02 SAFE, plus 15 SAFE deposited into the pool in exchange for
pool tokens.

## Step 2: Deposit SAFE into the pool

Now that the pool exists, let's deposit some SAFE in exchange for some pool tokens.

SAFE will likely be the most attractive form of deposit, since it's the easiest
...
We already deposited 15 SAFE during creation of the pool, but let's deposit 
another 10 SAFE into the pool:
...
Now that the pool has some SAFE, we need to add validators to it.
...
This operation moves 1.00228288 SAFE from the reserve to a stake account on a given
validator.  This means you'll need over 1 SAFE for each validator that you want to add.

## Step 4: Deposit stakes into the pool

Now that your pool has validators, it can accept stake accounts for you
to manage. There are two possible sources of deposits: SAFE or stake accounts.
In step 2, we deposited SAFE directly, so now we'll deposit stake accounts.
...
Over time, as people deposit SAFE into the reserve, or as validator performance
...
[stake-pool-py on GitHub](https://github.com/fair-exchange/safecoin-program-library/tree/master/stake-pool/py).
...
withdraw SAFE from the reserve if it has enough SAFE, or to withdraw from one of
the stake accounts in the pool.

The `withdraw.sh` script removes stakes and SAFE from each of the stake accounts
