...
* Epoch: every epoch (roughly 2 days), the stake accounts in the pool earn 
  ...
  10 SAFE in rewards, and the fee is set to 2%, the manager will earn pool tokens
  worth 0.2 SAFE.
* SAFE withdraw: sends a proportion of the desired withdrawal amount to the manager
...
* SAFE deposit: converts the entire SAFE deposit into pool tokens, then sends a
...
During SAFE or stake deposits, the stake pool can redistribute a percentage of
...
The stake pool burned 2 pool tokens. In return, the stake pool sent SAFE to the
