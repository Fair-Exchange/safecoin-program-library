...
fn process_accept_offer(
    ...
    let is_native = *taker_src_mint.key == safe_token::native_mint::id();
    if is_native {
    ...
    let maker_src_token_account: safe_token::state::Account =
        safe_token::state::Account::unpack(&maker_src_account.data.borrow())?;
    ...
    assert_keys_equal(safe_token::id(), *token_program_info.key)?;
    ...
    if *taker_src_mint.key == safe_token::native_mint::id() {
        ...
        Some(creators) => {
            for creator in creators {
                ...
                if !is_native {
                    ...
                    if !seeds.is_empty() {
                        msg!("Maker cannot pay with native SAFE");
                        return Err(ProgramError::InvalidAccountData);
                    }
                    match system_program_info {
                        Some(sys_program_info) => {
