...
async fn fail_without_sol_deposit_authority_signature() {
    ...
    match error {
        TransactionError::InstructionError(_, InstructionError::Custom(error_index)) => {
            assert_eq!(
                error_index,
                error::StakePoolError::InvalidSafeDepositAuthority as u32
            );
        }
        _ => panic!("Wrong error occurs while trying to make a deposit without SAFE deposit authority signature"),
    }
}
