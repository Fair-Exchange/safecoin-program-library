...
pub async fn get_account(banks_client: &mut BanksClient, pubkey: &Pubkey) -> SafecoinAccount {
    banks_client
        .get_account(*pubkey)
        .await
        .expect("client error")
...
pub async fn create_mint(
    ...
    assert!(extension_types.is_empty() || program_id != &safe_token::id());
    ...
    for extension_type in extension_types {
        let instruction = match extension_type {
            ExtensionType::MintCloseAuthority =>
                safe_token_2022::instruction::initialize_mint_close_authority(
                    program_id,
                    &mint_pubkey,
                    Some(manager),
                ),
            ExtensionType::DefaultAccountState =>
                safe_token_2022::extension::default_account_state::instruction::initialize_default_account_state(
                    program_id,
                    &mint_pubkey,
                    &safe_token_2022::state::AccountState::Initialized,
                ),
            ExtensionType::TransferFeeConfig => safe_token_2022::extension::transfer_fee::instruction::initialize_transfer_fee_config(
                program_id,
                &mint_pubkey,
                Some(manager),
                Some(manager),
            ...
            ExtensionType::InterestBearingConfig => safe_token_2022::extension::interest_bearing_mint::instruction::initialize(
                program_id,
                &mint_pubkey,
                Some(*manager),
                600,
            ...
            ExtensionType::NonTransferable =>
                safe_token_2022::instruction::initialize_non_transferable_mint(program_id, &mint_pubkey),
            _ => unimplemented!(),
        };
        instructions.push(instruction.unwrap());
    }
    instructions.push(
        safe_token_2022::instruction::initialize_mint(
            program_id,
            &pool_mint.pubkey(),
            manager,
            None,
...
#[allow(clippy::too_many_arguments)]
pub async fn transfer_safe_tokens(
    banks_client: &mut BanksClient,
    payer: &Keypair,
    recent_blockhash: &Hash,
    program_id: &Pubkey,
    ...
    let transaction = Transaction::new_signed_with_payer(
        &[safe_token_2022::instruction::transfer_checked(
            program_id,
            source,
            mint,
            destination,
...
pub async fn create_token_account(
    ...
    instructions.push(
        safe_token_2022::instruction::initialize_account(
            program_id,
            &account.pubkey(),
            pool_mint,
            &authority.pubkey(),
    ...
    for extension in extensions {
        match extension {
            ...
            ExtensionType::CpiGuard => {
                signers.push(authority);
                instructions.push(
                    safe_token_2022::extension::cpi_guard::instruction::enable_cpi_guard(
                        program_id,
                        &account.pubkey(),
                        &authority.pubkey(),
                        &[],
...
pub async fn close_token_account(
    ...
    let mut transaction = Transaction::new_with_payer(
        &[safe_token_2022::instruction::close_account(
            program_id,
            account,
...
pub async fn freeze_token_account(
    ...
    let mut transaction = Transaction::new_with_payer(
        &[safe_token_2022::instruction::freeze_account(
            program_id,
            account,
            pool_mint,
            &manager.pubkey(),
...
pub async fn mint_tokens(
    ...
    let transaction = Transaction::new_signed_with_payer(
        &[safe_token_2022::instruction::mint_to(
            program_id,
            mint,
            account,
...
pub async fn burn_tokens(
    ...
    let transaction = Transaction::new_signed_with_payer(
        &[safe_token_2022::instruction::burn(
            program_id,
            account,
            mint,
            &authority.pubkey(),
...
pub async fn delegate_tokens(
    ...
    let transaction = Transaction::new_signed_with_payer(
        &[safe_token_2022::instruction::approve(
            program_id,
            account,
            delegate,
            &manager.pubkey(),
...
pub async fn revoke_tokens(
    ...
    let transaction = Transaction::new_signed_with_payer(
        &[
            safe_token_2022::instruction::revoke(program_id, account, &manager.pubkey(), &[])
                .unwrap(),
        ],
        Some(&payer.pubkey()),
        &[payer, manager],
    ...
    let mut transaction = Transaction::new_with_payer(
        &[
            ...
            instruction::set_fee(
                ...
                &stake_pool.pubkey(),
                &manager.pubkey(),
                FeeType::SafeDeposit(*sol_deposit_fee),
            ),
            instruction::set_fee(
                &id(),
                &stake_pool.pubkey(),
                &manager.pubkey(),
                FeeType::SafeReferral(sol_referral_fee),
            ),
        ],
        Some(&payer.pubkey()),
    );
...
pub fn add_vote_account(program_test: &mut ProgramTest) -> Pubkey {
    ...
    let vote_account = SafecoinAccount::create(
...
pub fn add_validator_stake_account(
    ...
    let stake_account = SafecoinAccount::create(
...
pub fn add_reserve_stake_account(
    ...
    let reserve_stake_account = SafecoinAccount::create(
        stake_amount + STAKE_ACCOUNT_RENT_EXEMPTION,
...
pub fn add_stake_pool_account(
    ...
    stake_pool_bytes.extend_from_slice(Pubkey::default().as_ref());
    let stake_pool_account = SafecoinAccount::create(
        ACCOUNT_RENT_EXEMPTION,
        stake_pool_bytes,
        id(),
        false,
...
pub fn add_validator_list_account(
    ...
    let validator_list_account = SafecoinAccount::create(
        ACCOUNT_RENT_EXEMPTION,
        validator_list_bytes,
        id(),
        false,
...
pub fn add_mint_account(
    ...
    let stake_pool_mint = SafecoinAccount::create(
        ACCOUNT_RENT_EXEMPTION,
        mint_vec,
        *program_id,
        false,
...
pub fn add_token_account(
    ...
    let fee_account = SafecoinAccount::create(
        ACCOUNT_RENT_EXEMPTION,
        fee_account_vec,
        *program_id,
        false,
