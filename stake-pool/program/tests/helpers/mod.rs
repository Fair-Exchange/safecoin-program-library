...
use {
    ...
    safecoin_program_test::{processor, BanksClient, ProgramTest},
    safecoin_sdk::{
...
pub async fn freeze_token_account(
    ...
    let mut transaction = Transaction::new_with_payer(
        &[safe_token::instruction::freeze_account(
            &safe_token::id(),
            account,
            pool_mint,
            &manager.pubkey(),
...
pub fn add_validator_stake_account(
    ...
    let active_stake_lamports = stake_amount - LAMPORTS_PER_SAFE;
...
pub fn add_mint_account(
    ...
    let mut mint_vec = vec![0u8; safe_token::state::Mint::LEN];
    let mint = safe_token::state::Mint {
...
pub fn add_token_account(
    ...
    let mut fee_account_vec = vec![0u8; safe_token::state::Account::LEN];
    let fee_account_data = safe_token::state::Account {
        mint: *mint_key,
        owner: *owner,
        amount: 0,
