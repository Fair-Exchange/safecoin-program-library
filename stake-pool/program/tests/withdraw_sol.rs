...
async fn fail_overdraw_reserve() {
    ...
    assert_eq!(
        ...
        TransactionError::InstructionError(
            0,
            InstructionError::Custom(StakePoolError::SafeWithdrawalTooLarge as u32)
        )
    );
}
...
async fn fail_without_sol_withdraw_authority_signature() {
    ...
    assert_eq!(
        ...
        TransactionError::InstructionError(
            0,
            InstructionError::Custom(StakePoolError::InvalidSafeWithdrawAuthority as u32)
        )
    );
}
