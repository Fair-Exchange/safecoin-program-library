...
impl Processor {
    ...
    fn process_initialize(
        ...
        check_safe_token_program_account(token_program_info.key)?;
    ...
    /// Processes [DepositSafe](enum.Instruction.html).
    #[inline(never)] // needed to avoid stack size violation
    ...
    /// Processes [WithdrawSafe](enum.Instruction.html).
    #[inline(never)] // needed to avoid stack size violation
    fn process_withdraw_sol(
        ...
        if let stake::state::StakeState::Initialized(meta) = stake_state {
            ...
            if new_reserve_lamports < minimum_reserve_lamports {
                msg!("Attempting to withdraw {} lamports, maximum possible SAFE withdrawal is {} lamports",
                    withdraw_lamports,
                    reserve_stake_info.lamports().saturating_sub(minimum_reserve_lamports)
                );
                return Err(StakePoolError::SafeWithdrawalTooLarge.into());
            }
        } else {
            msg!("Reserve stake account not in intialized state");
            return Err(StakePoolError::WrongStakeState.into());
    ...
    fn process_set_funding_authority(
        ...
        match funding_type {
            ...
            FundingType::SafeDeposit => stake_pool.sol_deposit_authority = new_authority,
            FundingType::SafeWithdraw => stake_pool.sol_withdraw_authority = new_authority,
        }
        stake_pool.serialize(&mut *stake_pool_info.data.borrow_mut())?;
        Ok(())
    }
    ...
    pub fn process(program_id: &Pubkey, accounts: &[AccountInfo], input: &[u8]) -> ProgramResult {
        ...
        match instruction {
            ...
            StakePoolInstruction::DepositSafe(lamports) => {
                msg!("Instruction: DepositSafe");
                Self::process_deposit_sol(program_id, accounts, lamports, None)
            }
            StakePoolInstruction::WithdrawSafe(pool_tokens) => {
            ...
            StakePoolInstruction::DepositSafeWithSlippage {
                lamports_in,
                minimum_pool_tokens_out,
            } => {
                msg!("Instruction: DepositSafeWithSlippage");
            ...
            StakePoolInstruction::WithdrawSafeWithSlippage {
                pool_tokens_in,
                minimum_lamports_out,
            } => {
                msg!("Instruction: WithdrawSafeWithSlippage");
...
impl PrintProgramError for StakePoolError {
        ...
        E: 'static + std::error::Error + DecodeError<E> + PrintProgramError + FromPrimitive,
    {
        match self {
            ...
            StakePoolError::InvalidSafeDepositAuthority => msg!("Error: Provided sol deposit authority does not match the program's"),
