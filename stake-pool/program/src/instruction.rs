...
use {
    ...
    lpl_token_metadata::pda::find_metadata_account,
    solana_program::{
...
pub enum StakePoolInstruction {
    ...
    ///   Deposit SAFE directly into the pool's reserve account. The output is a "pool" token
    ///   representing ownership into the pool. Inputs are converted to the current ratio.
    ///
    ///   0. `[w]` Stake pool
    ///   1. `[]` Stake pool withdraw authority
    ///   2. `[w]` Reserve stake account, to deposit SAFE
    ...
    ///  (Manager only) Update SAFE deposit, stake deposit, or SAFE withdrawal authority.
    ///
    ///  0. `[w]` StakePool
    ///  1. `[s]` Manager
    ...
    ///   Withdraw SAFE directly from the pool's reserve account. Fails if the
    ///   reserve does not have enough SAFE.
    ///
    ///   0. `[w]` Stake pool
    ///   1. `[]` Stake pool withdraw authority
    ///   2. `[s]` User transfer authority, for pool token account
    ///   3. `[w]` User account to burn pool tokens
    ///   4. `[w]` Reserve stake account, to withdraw SAFE
    ...
    CreateTokenMetadata {
        /// Token name
        name: String,
        /// Token symbol e.g. stkSAFE
        symbol: String,
        /// URI of the uploaded metadata of the safe-token
        uri: String,
    },
    ...
    ///   Deposit SAFE directly into the pool's reserve account, with a specified
    ...
    ///   Withdraw SAFE directly from the pool's reserve account. Fails if the
    ///   reserve does not have enough SAFE or if the slippage constraint is not
    ///   met.
    ///
    ///   0. `[w]` Stake pool
    ///   1. `[]` Stake pool withdraw authority
...
/// Creates instructions required to deposit SAFE directly into a stake pool.
fn deposit_sol_internal(
    ...
    if let Some(minimum_pool_tokens_out) = minimum_pool_tokens_out {
        Instruction {
            program_id: *program_id,
            accounts,
            data: StakePoolInstruction::DepositSafeWithSlippage {
                lamports_in,
                minimum_pool_tokens_out,
            }
            .try_to_vec()
        ...
        Instruction {
            program_id: *program_id,
            accounts,
            data: StakePoolInstruction::DepositSafe(lamports_in)
                .try_to_vec()
                .unwrap(),
        }
    }
...
/// Creates instruction to deposit SAFE directly into a stake pool.
pub fn deposit_sol(
    program_id: &Pubkey,
    stake_pool: &Pubkey,
    stake_pool_withdraw_authority: &Pubkey,
...
/// Creates instruction to deposit SAFE directly into a stake pool with slippage constraint.
...
/// Creates instruction required to deposit SAFE directly into a stake pool.
...
fn withdraw_sol_internal(
    ...
    if let Some(minimum_lamports_out) = minimum_lamports_out {
        Instruction {
            program_id: *program_id,
            accounts,
            data: StakePoolInstruction::WithdrawSafeWithSlippage {
                pool_tokens_in,
                minimum_lamports_out,
            }
            .try_to_vec()
        ...
        Instruction {
            program_id: *program_id,
            accounts,
            data: StakePoolInstruction::WithdrawSafe(pool_tokens_in)
                .try_to_vec()
                .unwrap(),
        }
    }
...
/// Creates instruction required to withdraw SAFE directly from a stake pool.
pub fn withdraw_sol(
    program_id: &Pubkey,
    stake_pool: &Pubkey,
    stake_pool_withdraw_authority: &Pubkey,
...
/// Creates instruction required to withdraw SAFE directly from a stake pool with
