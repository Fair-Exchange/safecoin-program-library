...
# Script to withdraw stakes and SAFE from a stake pool, given the stake pool public key
...
create_stake_account () {
  ...
  do
    safecoin-keygen new --no-passphrase -o "$keys_dir/stake_account_$validator.json"
    safecoin create-stake-account "$keys_dir/stake_account_$validator.json" 2 
    safecoin delegate-stake --force "$keys_dir/stake_account_$validator.json"  "$validator" 
  done < "$validator_list"
}
...
withdraw_stakes_to_stake_receiver () {
  ...
  while read -r validator
  do
    stake_receiver=$(safecoin-keygen pubkey "$keys_dir/stake_account_$validator.json")
...
echo "Withdrawing SAFE from stake pool to authority"
