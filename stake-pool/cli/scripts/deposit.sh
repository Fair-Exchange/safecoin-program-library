...
# Script to deposit stakes and SAFE into a stake pool, given the stake pool keyfile
...
create_user_stakes () {
  ...
  do
    ...
    safecoin create-stake-account "$keys_dir/stake_$validator.json" "$sol_amount" --withdraw-authority "$authority" --stake-authority "$authority"
  done < "$validator_list"
}
...
delegate_user_stakes () {
  ...
  while read -r validator
  do
    safecoin delegate-stake --force "$keys_dir/stake_$validator.json" "$validator" --stake-authority "$authority"
  done < "$validator_list"
}
...
echo "Depositing SAFE into stake pool"
