...
fn command_list(config: &Config, stake_pool_address: &Pubkey) -> CommandResult {
    ...
    if config.verbose {
        ...
        println!("SAFE Deposit Authority: {}", sol_deposit_authority);
    ...
    if stake_pool.sol_deposit_fee.numerator > 0 && stake_pool.sol_deposit_fee.denominator > 0 {
        println!(
            "SAFE Deposit Fee: {} of deposit amount",
            stake_pool.sol_deposit_fee
        );
    } else {
        println!("SAFE Deposit Fee: none");
    }
    if stake_pool.sol_referral_fee > 0 {
        println!(
            "SAFE Deposit Referral Fee: {}% of SAFE Deposit Fee",
            stake_pool.sol_referral_fee
        );
    } else {
        println!("SAFE Deposit Referral Fee: none");
    }
