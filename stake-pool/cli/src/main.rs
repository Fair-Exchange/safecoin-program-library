...
#[allow(deprecated)]
use safe_associated_token_account::create_associated_token_account;
...
fn command_deposit_sol(
    ...
    if from_balance < amount {
        return Err(format!(
            "Not enough SAFE to deposit into pool: {}.\nMaximum deposit amount is {} SAFE.",
            Safe(amount),
            Safe(from_balance)
        )
        .into());
    ...
    let deposit_instruction = if let Some(deposit_authority) = config.funding_authority.as_ref() {
        let expected_sol_deposit_authority = stake_pool.sol_deposit_authority.ok_or_else(|| {
            "SAFE deposit authority specified in arguments but stake pool has none".to_string()
        })?;
    ...
    let withdraw_instruction = if let Some(withdraw_authority) = config.funding_authority.as_ref() {
        let expected_sol_withdraw_authority =
            stake_pool.sol_withdraw_authority.ok_or_else(|| {
                "SAFE withdraw authority specified in arguments but stake pool has none".to_string()
            })?;
...
fn main() {
    ...
    let matches = App::new(crate_name!())
        ...
        .subcommand(SubCommand::with_name("decrease-validator-stake")
            ...
            .arg(
                Arg::with_name("amount")
                    ...
                    .value_name("AMOUNT")
                    .takes_value(true)
                    .help("Amount in SAFE to remove from the validator stake account. Must be at least the rent-exempt amount for a stake."),
            )
        )
        ...
        .subcommand(SubCommand::with_name("deposit-sol")
            .about("Deposit SAFE into the stake pool in exchange for pool tokens")
            .arg(
                Arg::with_name("pool")
                    .index(1)
                    .validator(is_pubkey)
                ...
                Arg::with_name("amount")
                    ...
                    .value_name("AMOUNT")
                    .takes_value(true)
                    .help("Amount in SAFE to deposit into the stake pool reserve account."),
            )
            .arg(
                Arg::with_name("from")
                    .long("from")
        ...
        .subcommand(SubCommand::with_name("withdraw-sol")
            .about("Withdraw SAFE from the stake pool's reserve in exchange for pool tokens")
            .arg(
                Arg::with_name("pool")
                    .index(1)
                    .validator(is_pubkey)
            ...
            .arg(
                Arg::with_name("sol_receiver")
                    ...
                    .takes_value(true)
                    .required(true)
                    .help("System account to receive SAFE from the stake pool. Defaults to the payer."),
            )
            .arg(
                Arg::with_name("amount")
                    .index(3)
                    ...
                    .takes_value(true)
                    .required(true)
                    .help("Amount of pool tokens to withdraw for SAFE."),
            )
            .arg(
                Arg::with_name("pool_account")
                    .long("pool-account")
    ...
    let _ = match matches.subcommand() {
        ...
        ("set-funding-authority", Some(arg_matches)) => {
            ...
            let funding_type = match arg_matches.value_of("funding_type").unwrap() {
                "sol-deposit" => FundingType::SafeDeposit,
                "stake-deposit" => FundingType::StakeDeposit,
                "sol-withdraw" => FundingType::SafeWithdraw,
                _ => unreachable!(),
            };
            let _unset = arg_matches.is_present("unset");
        ...
        ("set-fee", Some(arg_matches)) => {
            ...
            match arg_matches.value_of("fee_type").unwrap() {
                ...
                "sol-deposit" => {
                    command_set_fee(&config, &stake_pool_address, FeeType::SafeDeposit(new_fee))
                }
                ...
                "sol-withdrawal" => command_set_fee(
                    &config,
                    &stake_pool_address,
                    FeeType::SafeWithdrawal(new_fee),
                ),
                _ => unreachable!(),
            }
        }
