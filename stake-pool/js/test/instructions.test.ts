...
describe('StakePoolProgram', () => {
  const connection = new Connection('http://127.0.0.1:8328');

  connection.getMinimumBalanceForRentExemption = jest.fn(async () => 10000);

  const stakePoolAddress = new PublicKey('spNkUopdHp8YxSdsajeNuSKZm51w9kPt48SDDPSHFvo');
  ...
  it('StakePoolInstruction.depositSafe', () => {
    const payload: DepositSafeParams = {
    ...
    const instruction = StakePoolInstruction.depositSafe(payload);
    ...
    const decodedData = decodeData(STAKE_POOL_INSTRUCTION_LAYOUTS.DepositSafe, instruction.data);

    expect(decodedData.instruction).toEqual(STAKE_POOL_INSTRUCTION_LAYOUTS.DepositSafe.index);
    expect(decodedData.lamports).toEqual(payload.lamports);

    payload.depositAuthority = Keypair.generate().publicKey;

    const instruction2 = StakePoolInstruction.depositSafe(payload);
  ...
  describe('depositSafe', () => {
    ...
    it.only('should throw an error with invalid balance', async () => {
      await expect(depositSafe(connection, stakePoolAddress, from, balance + 1)).rejects.toThrow(
        Error('Not enough SAFE to deposit into pool. Maximum deposit amount is 0.00001 SAFE.'),
      );
    });

    it.only('should throw an error with invalid account', async () => {
      connection.getAccountInfo = jest.fn(async () => null);
      await expect(depositSafe(connection, stakePoolAddress, from, balance)).rejects.toThrow(
    ...
    it.only('should call successfully', async () => {
      ...
      const res = await depositSafe(connection, stakePoolAddress, from, balance);
  ...
  describe('withdrawSafe', () => {
    ...
    it.only('should throw an error with invalid stake pool account', async () => {
      connection.getAccountInfo = jest.fn(async () => null);
      await expect(
        withdrawSafe(connection, stakePoolAddress, tokenOwner, solReceiver, 1),
    ...
    it.only('should call successfully', async () => {
      connection.getAccountInfo = jest.fn(async (pubKey: PublicKey) => {
        ...
        if (pubKey.equals(CONSTANTS.poolTokenAccount)) {
          return mockTokenAccount(LAMPORTS_PER_SAFE);
        }
        return null;
      });
      const res = await withdrawSafe(connection, stakePoolAddress, tokenOwner, solReceiver, 1);
  ...
  describe('withdrawStake', () => {
    ...
    it.only('should call successfully', async () => {
      connection.getAccountInfo = jest.fn(async (pubKey: PublicKey) => {
        ...
        if (pubKey.equals(CONSTANTS.poolTokenAccount)) {
          return mockTokenAccount(LAMPORTS_PER_SAFE * 2);
        }
