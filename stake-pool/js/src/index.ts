...
import { ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, Token } from '@safecoin/safe-token';
import {
  ValidatorAccount,
...
export async function depositSafe(
  ...
  if (fromBalance < lamports) {
    throw new Error(
      `Not enough SAFE to deposit into pool. Maximum deposit amount is ${lamportsToSafe(
        fromBalance,
      )} SAFE.`,
    );
  }
  ...
  // Ephemeral SAFE account just to do the transfer
  const userSafeTransfer = new Keypair();
  const signers: Signer[] = [userSafeTransfer];
  const instructions: TransactionInstruction[] = [];

  let rentFee = 0;

  // Create the ephemeral SAFE account
  instructions.push(
    SystemProgram.transfer({
      fromPubkey: from,
      toPubkey: userSafeTransfer.publicKey,
      lamports,
    }),
  );
  ...
  instructions.push(
    StakePoolInstruction.depositSafe({
      stakePool: stakePoolAddress,
      reserveStake: stakePool.reserveStake,
      fundingAccount: userSafeTransfer.publicKey,
...
export async function withdrawStake(
  ...
  if (tokenAccount.amount.toNumber() < poolAmount) {
    throw new Error(
      `Not enough token balance to withdraw ${lamportsToSafe(poolAmount)} pool tokens.
        Maximum withdraw amount is ${lamportsToSafe(tokenAccount.amount.toNumber())} pool tokens.`,
    );
  }
...
/**
 * Creates instructions required to withdraw SAFE directly from a stake pool.
 */
export async function withdrawSafe(
  ...
  if (solWithdrawAuthority) {
    const expectedSafeWithdrawAuthority = stakePool.account.data.solWithdrawAuthority;
    if (!expectedSafeWithdrawAuthority) {
      throw new Error('SAFE withdraw authority specified in arguments but stake pool has none');
    }
    if (solWithdrawAuthority.toBase58() != expectedSafeWithdrawAuthority.toBase58()) {
      throw new Error(
        `Invalid deposit withdraw specified, expected ${expectedSafeWithdrawAuthority.toBase58()}, received ${solWithdrawAuthority.toBase58()}`,
      );
    }
  }

  const withdrawTransaction = StakePoolInstruction.withdrawSafe({
    stakePool: stakePoolAddress,
...
export async function stakePoolInfo(connection: Connection, stakePoolAddress: PublicKey) {
  ...
  const totalPoolTokens = lamportsToSafe(stakePool.account.data.poolTokenSupply);
  ...
  return {
    ...
    nextSafeWithdrawalFee: stakePool.account.data.nextSafeWithdrawalFee,
