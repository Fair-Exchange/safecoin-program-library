 ...
 */
export type WithdrawSafeParams = {
  stakePool: PublicKey;
  sourcePoolAccount: PublicKey;
  withdrawAuthority: PublicKey;
  reserveStake: PublicKey;
...
export class StakePoolInstruction {
  ...
  static depositSafe(params: DepositSafeParams): TransactionInstruction {
    ...
    const type = STAKE_POOL_INSTRUCTION_LAYOUTS.DepositSafe;
   ...
   */
  static withdrawSafe(params: WithdrawSafeParams): TransactionInstruction {
    const {
      stakePool,
      withdrawAuthority,
      sourceTransferAuthority,
    ...
    const type = STAKE_POOL_INSTRUCTION_LAYOUTS.WithdrawSafe;
    const data = encodeData(type, { poolTokens });
   ...
   */
  static decodeDepositSafe(instruction: TransactionInstruction): DepositSafeParams {
    this.checkProgramId(instruction.programId);
    this.checkKeyLength(instruction.keys, 9);

    const { amount } = decodeData(STAKE_POOL_INSTRUCTION_LAYOUTS.DepositSafe, instruction.data);
