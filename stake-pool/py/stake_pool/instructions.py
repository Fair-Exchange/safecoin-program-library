...
class RemoveValidatorFromPoolParams(NamedTuple):
    ...
    """`[w]` Destination stake account, to receive the minimum SAFE from the validator stake account."""
    clock_sysvar: PublicKey
    """'[]' Stake config sysvar."""
    stake_program_id: PublicKey
    """`[]` Stake program."""
...
class DepositSafeParams(NamedTuple):
...
class WithdrawSafeParams(NamedTuple):
    """Withdraw SAFE directly from the pool's reserve account."""
...
INSTRUCTIONS_LAYOUT = Struct(
    ...
    / Switch(
        lambda this: this.instruction_type,
        {
            ...
            InstructionType.DEPOSIT_SAFE: AMOUNT_LAYOUT,
            InstructionType.SET_FUNDING_AUTHORITY: Pass,  # TODO
            InstructionType.WITHDRAW_SAFE: AMOUNT_LAYOUT,
        },
    ),
)
...
def deposit_sol(params: DepositSafeParams) -> TransactionInstruction:
    """Creates a transaction instruction to deposit SAFE into a stake pool."""
    keys = [
        AccountMeta(pubkey=params.stake_pool, is_signer=False, is_writable=True),
        AccountMeta(pubkey=params.withdraw_authority, is_signer=False, is_writable=False),
    ...
    return TransactionInstruction(
        ...
        data=INSTRUCTIONS_LAYOUT.build(
            dict(
                instruction_type=InstructionType.DEPOSIT_SAFE,
                args={'amount': params.amount}
            )
        )
    )
...
def withdraw_sol(params: WithdrawSafeParams) -> TransactionInstruction:
    """Creates a transaction instruction to withdraw SAFE from a stake pool."""
    keys = [
        AccountMeta(pubkey=params.stake_pool, is_signer=False, is_writable=True),
        AccountMeta(pubkey=params.withdraw_authority, is_signer=False, is_writable=False),
    ...
    return TransactionInstruction(
        ...
        data=INSTRUCTIONS_LAYOUT.build(
            dict(
                instruction_type=InstructionType.WITHDRAW_SAFE,
                args={'amount': params.amount}
            )
        )
    )
