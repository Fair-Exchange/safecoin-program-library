...
LAMPORTS_PER_SAFE: int = 1_000_000_000
MINIMUM_INCREASE_LAMPORTS: int = LAMPORTS_PER_SAFE // 100
...
async def rebalance(endpoint: str, stake_pool_address: PublicKey, staker: Keypair, retained_reserve_amount: float):
    ...
    retained_reserve_lamports = int(retained_reserve_amount * LAMPORTS_PER_SAFE)
...
if __name__ == "__main__":
    ...
    parser.add_argument('reserve_amount', metavar='RESERVE_AMOUNT', type=float,
                        help='Amount of SAFE to keep in the reserve, e.g. 10.5')
    parser.add_argument('--endpoint', metavar='ENDPOINT_URL', type=str,
                        default='https://api.mainnet-beta.safecoin.org',
                        help='RPC endpoint to use, e.g. https://api.mainnet-beta.safecoin.org')
    ...
    print(f'Amount to leave in the reserve: {args.reserve_amount} SAFE')
