...
import spl.token.instructions as safe_token
...
async def create_associated_token_account(
    ...
    txn = Transaction()
    create_txn = safe_token.create_associated_token_account(
...
async def create_mint(client: AsyncClient, payer: Keypair, mint: Keypair, mint_authority: PublicKey):
    ...
    txn.add(
        safe_token.initialize_mint(
            safe_token.InitializeMintParams(
                program_id=TOKEN_PROGRAM_ID,
