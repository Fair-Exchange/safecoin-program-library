...
Note: the tests all run against a `safecoin-test-validator` with short epochs of 64
...
```
$ flake8 bot safe_token stake stake_pool system tests vote
```

### Type Checker

```
$ mypy bot stake stake_pool tests vote safe_token system
```
...
uniform, while also maintaining some SAFE in the reserve if desired. Can be run
with the stake pool address, staker keypair, and SAFE to leave in the reserve:
