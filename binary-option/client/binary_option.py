...
ASSOCIATED_TOKEN_ACCOUNT_PROGRAM_ID = 'AToD9iqHSc2fhEP9Jp7UYA6mRjHQ4CTWyzCsw8X3tH7K'
TOKEN_PROGRAM_ID = 'ToKLx75MGim1d1jRusuVX8xvdvvbSDESVaNXpRA9PHN'
BINARY_OPTION_PROGRAM_ID = 'optUzh7ifoZWFvo6RvAc4CUfTuWkciUCtS6yLFCP8gD'
        ...
        try:
            ...
            try:
                if amount is None:
                    ...
                    lamports = int(amount)
                msg += f" | Fetched lamports: {lamports * 1e-9} SAFE"
            except Exception as e:
            ...
            try:
                ...
                return json.dumps(
                    {
                        'status': HTTPStatus.OK,
                        'msg': f"Successfully sent {lamports * 1e-9} SAFE to {to}",
                        'tx': response.get('result') if skip_confirmation else response['result']['transaction']['signatures'],
                    }
                )
            except Exception as e:
